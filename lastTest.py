...
def fit_training():
    ...
    # dont need to do a train test split
...
@st.cache(allow_output_mutation=True)
def test_model():
    tfidf_test_matrix = tfidf.fit_transform(test_df['text'])
    test_pred = model.predict_proba(tfidf_test_matrix)
    test_pred_int = test_pred[:, 1] >= 0.5
    test_pred_int = test_pred_int.astype(np.int)
    test_df['prediction'] = test_pred_int
    test_df[test_df['prediction'] == 0].count()

    # Computing F1 score for model on test data
    TP = (test_df['sentiment'] & test_df['prediction']).sum()
    FP = (~test_df['sentiment'] & test_df['prediction']).sum()
    FN = (test_df['sentiment'] & ~test_df['prediction']).sum()
    precision = TP / (TP + FP)
    recall = TP / (TP + FN)
...
if input_type == "Type or Paste a Tweet":
    ...
    if user_link == "":
        ...
        id = str(user_link[-19:])
        bearer_token = 'AAAAAAAAAAAAAAAAAAAAADBZKAEAAAAADD5O1x97HNJhBKaG102eVysU4F8%3DDMC4Ic3hqCxBDA3nARRgeWtRFQhKdoUoNEJokNB3PjOpyZwNLO'
        url = create_url(id)
    ...
    user_handle = st.text_input(label="Enter the handle of the user whose last tweet you want to analyze")
    if user_handle == "":
        pass
    else:
